"""
[목표]
1. 문장이 숫자 벡터로 바뀌는 과정과 Vector라는 공간에 대한 이해
2. Vector 공간에 저장된 데이터 간의 유사도인 "코사인 유사도" 이해
3. 기존 SQL 구문에서는 LIKE로 단어 기반의 검색을 했다면 유사도에 따른 의미 기반의 검색 이해

"""

# numpy는 여러 개의 숫자를 배열로 다루고 해당 숫자들을 빠르게 계산할 수 있는 파이썬 라이브러리이다.
# 임베딩의 결과는 수백 개의 숫자로 이루어진 배열 형태이다.
# 따라서 numpy를 활용해서 임베딩 결과를 저장하고 계산하는 것에 사용할 것이다.
import numpy as np
# HuggingFace에 있는 임베딩 Model을 활용한다.
