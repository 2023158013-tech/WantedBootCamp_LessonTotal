...
3. Chunk로 잘린 Text를 임베딩 후 VectorDB에 저장하고 검색한다.

-많은 양의 Text를 통채로 임베딩하게 되면 1개의 벡터에 여러 주제가 섞여 유사도를 판단하기 좋지 않은 벡터값이 된다.
-따라서 Text를 주제/핵심 키워드 등을 기준으로 잘라(청킹) 각각 임베딩을 해야 퀄리티 있는 답변을 만들 수 있다.
"""

# csv 파일을 읽기 위한 준비
import csv
# Path는 운영체제에 맞는 파일 경로를 만들어주는 도구이다.
from pathlib import Path

import chromadb
# 문서가 들어오면 지정한 모델로 임베딩하라고 명령한다.
from chromadb.utils import embedding_functions

# pdf 읽는 라이브러리
from pypdf import PdfReader

# Step 1. 사용 모델, DB 경로, Table명 설정, 청크 사이즈, 청크 오버랩 설정
# Step 1. 사용 모델, DB 경로, Table명 설정
...
CHUNK_SIZE = 500 # 1개의 청크에 포함되는 최대 글자수
CHUNK_OVERLAP = 50 # 이웃 청크끼리 겹치는 글자수
