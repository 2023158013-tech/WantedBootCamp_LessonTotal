...
어제의 RAG와는 조금 다른 점
* RAg: "출석 인정 기준이 뭐야?"라고 물어보면 답은 VectorDB에 넣어둔 규정.pdf 파일에 데이터가 존재한다.
* Function-Calling: "내가 듣고 있는 1번 강의의 진도율은 몇 % 야?"라고 물어보면 답은 MySQL 수강 이력 DB에 존재한다.

- 문서 등은 미리 임베딩 해둘 수 있지만 현재 학생의 진도율 데이터는 실시간 데이터 조회가 필요하다.
- 따라서 AI 모델(Google Gemini)에게 도구를 쥐어줄 것이다. (Function)
"""
...
# API Key, 인스턴스 주소는 env로 작성 후 git에는 올라가지 않게 설정하는 것이 중요하다.
SPRING_BASE_URL=os.getenv("SPRING_BASE_URL")

# 타임아웃 관련 설정
# FastAPI -> Spring 요청 시 Spring이 응답을 안준다면?
# 해당 함수는 영문도 모른채 영원히 기다리게 된다.
# 따라서 5초 안에 응답을 하지 않는다면 실패로 처리할 것이다.
TIMEOUT = httpx.Timeout(5.0)

# ==========================================================================
# FastAPI 쪽에서 LLM없이 직접 Spring API 호출
...
# Get 전용 통합 핸들러이기 때문에 URL만 전달할 수도 있고 필요하면 파라미터가 올수도 있다.
# 따라서 파라미터는 있으면 dict 타입으로 전달 받고 없으면 null로 처리한다.
def _spring_get(path:str, params:dict | None = None) -> dict:
    try:
        ...
        return resp.json() # spring api는 json 문자열을 return해준다.
        # json 문자열을 json 객체로 바꿔 주는 것이 json()이다.
...
if __name__ == "__main__":
    ...
    config = types.GenerateContentConfig(
        ...
        system_instruction=(
            "너는 LMS 학습 도우미이다."
            "현재 로그인한 학생의 ID는 1이다."   
            "학생의 개인 데이터가 필요하면 반드시 함수로 조회하고 조회 결과에 없는 내용은 지어내지 말아라."
            "함수가 error를 반환하면 그 상황을 정중하고 자세히 설명해라." 
        ),
        temperature=0.2
    )
