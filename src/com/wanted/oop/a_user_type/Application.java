...
public class Application {
    public static void main(String[] args) {
        ...
        *  2. 모든 회원 정보를 인자로 메소드를 호출 시 값을 전달해야 한다면 너무 많은 값들을 인자로 전달해야 하기 때문에 한 눈에 안 들어온다.
        ...
        *  new : 할당 연산자
        *  2. 사용자 정의의 자료형인 클래스를 생성하면, 클래스에 우리가 정의한 필드(전역 변수), 메소드대로 객체가 생성된다.
        *  */

        Member member = new Member();
        /*멤버가 가진 값에 접근하기 위해서는 인스턴스 변수명.필드명
        * 여기서 .은 참조연산자로서 레퍼런스 변수가 참조하고 있는 주소에 접근하겠다는 의미이다.*/
        System.out.println("member가 가진 이름: " + member.name); //String이라 기본값인 null이 출력됨
        System.out.println("member가 가진 나이: " + member.age); //int라 기본값인 0이 출력됨
