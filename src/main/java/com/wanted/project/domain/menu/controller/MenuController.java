...
@RequiredArgsConstructor //생성자 없어도 됨
@RequestMapping("/menu/**")
public class MenuController {
    ...
    public String regist() {
        return "menu/regist"; //뷰 이름
    }

    //해당 메서드는 비동기 방식으로 페이지를 리턴하는 것이 아닌 데이터만 리턴할 것이다.
    @GetMapping("/category")
    /*comment
    *  @ResponseBody를 붙이게 되면 웹페이지에 Json 형태로 데이터를 리턴하게 된다.
    *  Json은 JavaScript 객체 표기법으로 우리 Java 클래스와 비슷한 역할이라고 보면 된다.
    *  @ResponseBody는 1개의 페이지에 여러 데이터를 표현할 때 1개의 핸들러 메서드에서
    *  여러 데이터를 넣는 것이 아닌 비동기 방식으로 각 핸들러 메서드에서 전달되는 값을
    *  조합할 때 유용하게 사용된다.
    *  */
    @ResponseBody
