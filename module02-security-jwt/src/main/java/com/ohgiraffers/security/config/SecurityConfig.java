...
public class SecurityConfig {
    ...
    *  */
    ...
    /*comment
     *  8080 백엔드 서버와 5173 프론트 서버 연결 관련 설정
     *  CORS:cross origin relationship
     *  -> 서로 다른 origin 출처 간의 연결 설정 허용
     *  */
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOriginPatterns(Arrays.asList( //커스터마이징 포인트🛠️
        ...
        configuration.setAllowedHeaders(Arrays.asList(
                "Authorization", //중요하게 볼 부분(이 헤더 부분에 access token을 담을 것)
                "Content-Type",
                "X-Requested-With",
                "Accept",
                "Origin",
...
-     *
     */
    @Bean //AuthentificationManager 관련 Bean(영속성 컨텍스트의 매니저와 동일)
    public AuthenticationManager authenticationManager(AuthenticationConfiguration configuration) throws Exception {
        return configuration.getAuthenticationManager();
    }

    @Bean //PasswordEncoder 관련 Bean
    public BCryptPasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder();
    }

    /*인증/인가 관련 Security에서 가장 중요한 설정 Bean*/
    @Bean
        ...
        * */
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        //이곳은 1차 인증 인가 관련 방호벽이다.(커스터마이징 포인트🛠️)
                        // /api/user/ 하위에 endpoint 중에 admin/user 권한 별로 접근하게 하기 위해서는 메서드 레벨에서 2차로 방호벽을 구축한다.
                        .requestMatchers("/api/auth/**").permitAll() // 인증 없이 허용(1차 관문)
                        .requestMatchers("/api/users/**").hasAnyAuthority( "ROLE_USER") //1차 관문
                        .requestMatchers("/api/admin/**").hasAnyAuthority("ROLE_ADMIN") //1차 관문
