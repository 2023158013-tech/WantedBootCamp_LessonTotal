...
public class PerformanceAspect {
    ...
    *  해당 클래스 내부에서 관점을 어디(Pointcut)에 적용할지와 무엇(Advice)을 할지를 하나의 단위로 묶게 된다.
    *  */

    /*comment
    *  @Around 어노테이션은 메서드 실행 전과 후에 실행되는 Advice를 정의하는 어노테이션이다.
    ...
    *  -JoinPoint: Pointcut에 작성한 표현식을 바탕으로 실제 Advice가 동작하는 대상
    *
    *  */
    ...
    public Object measurePerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            /*comment
            *  ProceedingJoinPoint: @Around Advice에서 사용되는 특별한 인터페이스.
            *  일반적인 JoinPoint를 확장한 것으로 대상 메서드의 실행을 직접 제어할 수 있다.
            *  .proceed():  대상 메서드를 실행하는 메서드
            *  proceed()를 작성하지 않으면 대상(원본) 메서드는 실행되지 않는다. == 강력하다.
