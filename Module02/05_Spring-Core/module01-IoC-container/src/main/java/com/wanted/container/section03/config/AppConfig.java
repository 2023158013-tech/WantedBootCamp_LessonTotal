...
public class AppConfig {
    ...
    *  -private int a = 10;*/
    @Bean("singlePay") //메소드명이 아닌 직접 이름을 명시할 수 있다.(명시 안하면 메소드명이 이름)
    ...
    *  -주의: Bean이 생성되면 컨테이너는 더이상 관리하지 않는다. 따라서 메모리 해제 등을 별도로 고려해야 한다.
    *  */
    @Bean("protoPay")
    @Scope("prototype") //이 설정을 하게되면 더이상 싱글톤으로 관리되지 않는다.
