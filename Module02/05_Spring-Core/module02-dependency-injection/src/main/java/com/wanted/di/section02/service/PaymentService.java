...
public class PaymentService {
    ...
    /*comment
    *  @AutoWired: 스프링의 핵심 어노테이션 중 하나이며 객체간 의존성을 자동으로 주입해주는 역할을 하게 된다.
    *  ex) controller->service->dao
    *  -Service에서 PaymentInterface를 의존하려고 할 때 Error가 발생하는 이유는 PaymentInterface를 상속받은 구현체가 2개이기 때문에
    *  어떤 Bean을 주입해야 할 지 몰라서 발생하는 Error이다. */
    @Autowired
