...
public class EntityManagerLifeCycle {
    ...
    *  -엔티티를 저장, 수정, 삭제, 조회하는 등의 엔티티 관련 모든 일을 해준다.
    *  -엔티티 매니저는 thread-safe 하지 않기 때문에 동시성 문제가 발생할 수 있다.
    *  -고로, 1개의 요청 당 1개의 매니저를 통해 DB와의 연결을 맺어야 한다.
    *  영속성 컨텍스트란?
    *  -엔티티 매니저를 통해 엔티티를 저장, 조회를 하면 매니저는 영속성 컨텍스트에 엔티티를 보관 및 관리한다.
    *  -영속성 컨텍스트는 엔티티를 Key-Value 형태로 저장하며 매니저를 통해서만 접근 가능하고 관리할 수 있다.
    *  */
