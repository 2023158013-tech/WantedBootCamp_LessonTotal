...
public class ProblemExJDBC {
    ...
    /*comment
    *  테스트 코드 기반으로 JPA를 사용하지 않을 때
    *  발생할 수 있는 문제를 예시로 알아보자.
    *  */

    /*comment
    *  Test 전용 클래스란?
    *  @Test라는 어노테이션을 1개 이상 가지고 있는 클래스를 의미한다.
    *  테스트 메서드는 반환값이 없으며 void로 작성한다.
    *  메서드별로 독립적인 실행이 가능하며 우리가 만든 기능을 테스트하는 것에 초점을 맞춘다.
    *  또한 접근제한자는 사용하지 않아도 되며, 단 private는 허용되지 않는다.
    *  */
    ...
    } //db 설정 set은 기능 하나당 계속 필요(각 메서드가 동작할 때마다 필요)
    ...
    *  1. 데이터 변환(DTO), SQL 구문 작성, JDBC 코드 중복 작성 문제
    *  -개발 시간이 증가, 불필요한 코드 반복, 유지보수성 악화
    *  2. SQL에 의존적인 개발
    *  -ERD에 맞춘 개발 방식
    ...
    *  */
    ...
    @Test
    @DisplayName("직접 SQL문을 작성해서 메뉴 조회 시 문제 확인")
    void testDirectSQL() throws SQLException {
        ...
        List<Menu> menuList = new ArrayList<>(); //TDD 방식(테스트하면서 클래스 생성)
        ...
        /*comment
        *  Test 클래스는 기능 검증을 위한 클래스이다.
        *  Assertions 클래스는 Test를 위한 검증 메서드를 제공해준다.
        *  */

        //menuList가 NotNull이면 test 통과
        //그렇지 않으면 test 불통과
        Assertions.assertNotNull(menuList); //assertNull라면 null이면 통과

//        for(Menu menu : menuList) {
//            System.out.println(menu);
//        } //이 구문 축약 가능↓ (향상된 for문도 for-each)
        menuList.forEach(menu -> System.out.println(menu)); //람다 구문
        //menuList 중에 menu 하나 가져온 것
    ...
    *  이렇게 요구가 바뀌게 되면 데이터베이스 DDL 구문을 수정해야하며, SQL 구문 수정, Application 코드 수정이 필요하다.
    *  */

    /*comment JPA가 등장하게 된 결정적인 계기
    ...
    *  */
    ...
    void equalsTwoObject() throws SQLException {
        ...
        if(rset2.next()) {
            menu2 = new Menu();
            menu2.setMenuCode(rset.getInt("MENU_CODE"));
            menu2.setMenuName(rset.getString("MENU_NAME"));
        ...
        /*comment
        *  중요!!
        *  하나의 DB에서 동일한 행에 해당하는 데이터를 꺼냈는데 각기 다른 객체에 담았을 때 발생하는 동일성 보장 문제.
