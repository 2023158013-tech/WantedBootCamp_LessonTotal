...
/*comment
*  Mockito: Test용 객체를 만드는 것에 도움을 주는 라이브러리
*  -단위 테스트의 본질은 Spring 전체를 테스트하는 것이 아닌 일부 Bean을 불러와서 테스트 대상 메서드를 실행하는 것이다.
*  -따라서 Spring Context를 전체 로드하는 것은 리소스를 많이 낭비하는 행위이다.
*  -Mockito는 Spring Context를 전체 로드하는 것이 아닌 필요한 Bean만 Mock(가짜)로 생성해서 불필요한 리소스 낭비를 방지한다.
*  */
...
public class OrderServiceTest {
    ...
    *  */
    ...
    @Test
    void 결제_성공_시_주문_처리가_성공해야하는_테스트() {
        ...
        /*comment
        *  현재 테스트 대상은 OrderService이다.
        *  PaymentService는 부가적인 요소이며 OrderService에 영향을 주면 안된다.
        *  -when().thenReturn()
    ...
    void 결제_성공_시_이메일_발송_테스트() {
        ...
        verify(notificationService).sendEmail(eq(email), anyString());//가짜 객체를 넣어줌(eq: equal)
