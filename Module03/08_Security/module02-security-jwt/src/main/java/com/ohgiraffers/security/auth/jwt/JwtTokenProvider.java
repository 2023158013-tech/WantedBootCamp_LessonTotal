...
public class JwtTokenProvider {
    ...
    @Value("${jwt.secret}") //yml 파일에 작성한 키값을 가져오는 식
    ...
    // 🎯 AccessToken 생성(중요하게 볼 부분): 노출되는 토큰(🛠️커스터마이징 포인트)
    public String createAccessToken(Authentication authentication) {
        String username = authentication.getName(); //추가적으로 필요한 값을 더 꺼내올 수 있다.(꼭 필요한 값만 넣기(탈취되도 괜찮은 값))
        ...
        /*comment
        *  JWT.builder로 만들어지는 정보가 실제 Front에서 활용할 수 있는 로그인 유저 관련 정보이다.
        *  현재는 userId와 권한이 들어있어서 Front에서는 이 2개의 값을 바탕으로 활용할 수 있게 된다.
        *  */
        //확인하면 좋을 부분(핵심)
        return Jwts.builder()
                .subject(username) //위에서 더 받아오면 여기에도 추가해야 함.(payload를 어떻게 구성할지 커스터마이징🛠️)
