...
public class CertificateService {
    ...
    /*comment
    *  Transactional은 주요 속성들이 있다.
    *  -propagation: 전파의 의미를 가지고 있고 default value는 REQUIRED(상위 트랜잭션이 있다면 같이 묶인다.)
    *  REQUIRES_NEW(상위 트랜잭션과 분리된 독립적인 트랜잭션 경계를 만든다.)
    *  -rollbackFor: 예외는 크게 2가지가 있다.(언체크 예외/체크 예외)
    *  체크 예외 시에도 rollback을 하기 위해서 작성해야 한다.
