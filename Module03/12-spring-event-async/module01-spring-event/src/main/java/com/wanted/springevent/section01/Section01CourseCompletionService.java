...
public class Section01CourseCompletionService {
    ...
    *  CourseService는 고로 Enrollment와 certificate 도메인을 모두 의존해야 한다.
    *  ---
    *  수업 시에는 EnrollmentService, CertificateService는 생략하고 바로 Repository 계층을 호출하는 것으로 축약한다.
    *  */
    ...
    //강의 완료 메서드
    /*comment
    *  CourseService에서 수강 관련, 수료증 관련 로직을 컨트롤하고 있다.
    *  강의를 본다는 행위가 Course 도메인에서는 "주요" 기능이고
    *  수강 완료, 수료증 발급 행위는 "부가" 기능이다.*/
    @Transactional
    public void completeCourse(Long enrollmentId) {
        ...
        log.info("[section01] Course 서비스가 수강 완료 처리와 수료증 발급 처리를 수행함.");
