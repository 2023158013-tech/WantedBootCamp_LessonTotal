...
public class Section02CertificateEventListener {
    ...
    /*comment
    *  Listener는 Event를 구독하다가 Event가 발행되면 자동으로 실행된다.
    *  Event 발행자는 Event 처리자를 몰라도 협력을 할 수 있게 된다.
    *  */

    /*comment
    *  주의! 현재는 학습 관계로 repository를 가지고 있지만 추후 개발 시에는 service를 가지고 있는 구조로 변경해야 한다.
    *  */
    ...
    /*comment
    *  @EventListener
    *  -해당 어노테이션은 ApplicationEventPublisher에 등록된 이벤트를 구독하는 역할을 하게 된다.
    *  */
    @EventListener
    public void eventHandle(Section02CourseCompletedEvent event) {
        ...
        Certificate certificate = certificateRepository.save( //원래라면 service 호출
