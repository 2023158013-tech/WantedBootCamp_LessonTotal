...
public class Section02CourseCompletionService {
    ...
    /*comment
    *  Section02는 Course 도메인의 핵심 기능인 수강 완료 처리만 담당한다.
    *  수료증 발급 기능과 로그 기능은 핵심 기능이 아닌 후속 기능이라고 판단하고
    *  직접 비즈니스 로직으로 관리하는 것이 아닌 "수강 완료가 발생했다"라는 사실만 이벤트로 발생시킬 것이다.
    *  ---
    *  [중요한 점]
    *  객체와 객체 간의 의존성을 제거하는 행위에 있어서 Event가 만능처럼 보일 수 있다.
    *  의존성을 제거해서 모든 기능을 Event 방식으로 바꾸게 되면 @Transactional 경계가 무너지게 된다.
    *  따라서 Event를 만들 때 신경써야 하는 부분은 @Transactional의 전파 혹은 예외 발생 시 처리가 굉장히 중요해진다.
    *  */
    ...
    public void completeCourse(Long enrollmentId){
        ...
        //publisher를 이용해서 "수강 완료" 이벤트 발행(발행된 이벤트를 듣는 리스너 필요)
