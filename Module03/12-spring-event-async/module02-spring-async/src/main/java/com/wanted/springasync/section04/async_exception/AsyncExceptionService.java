...
public class AsyncExceptionService {
    ...
    @Async //작업이 비동기 방식임을 알려줌
    public void voidExceptionAsync() {

        log.info("[section04] void 반환형 비동기 메서드 호출됨.. thread={}", Thread.currentThread().getName());

        SleepUtils.sleep(1000L); //1초 시간 끌기

        throw new IllegalArgumentException("❌void 반환 타입의 비동기 메서드 예외 발생!!❌"); //일부러 예외 발생시키기
        //무조건적으로 예외를 발생시켰기 때문에 로그 구문 사용 불가

//        log.info("[section04] void 반환형 비동기 메서드 종료됨.. thread={}", Thread.currentThread().getName());

    }

    public AsyncRetryJobResponse createRetryDemoJob(Long userId, int failUntilAttempt, int maxRetryCount) {

        //해당 메서드는 임시 데이터 Insert용 메서드이다.
