...
public class CourseInputView {
    ...
    *  */
    ...
    private void createCourseWithDefaultSection() {
        ...
        /*comment
        *  강의를 등록함과 동시에 강의 안에 sections을 삽입
        *  1. 강의를 등록한다.
        *  2. 강의 내부에 섹션을 등록한다.
        *
        * */
        ...
        if (result) {
            outputView.printSuccess("✅강좌와 기본 섹션이 성공적으로 생성되었습니다!");
        } else {
            outputView.printError("🚨강좌 개설 비지니스 로직 처리 중 문제 발생!!");
        }
    }
    ...
    private void createCourse() {
        ...
        /*comment
        *  select문 같은 경우에는 ResultSet 객체에 SQL 결과가 담기게 된다.
        *  그러면 Update, Insert, Delete는 어떻게 결과가 도출될까?
        *  DML 구문은 SQL을 execute하게 되면 영향을 받은 행의 갯수만큼 정수값을 리턴해준다.
        *  ex) 1개 행 삽입 완료 -> 1*/
        Long result = controller.createCourse(title, description);

        if (result != null && result > 0) { //삽입이 되었다.
    ...
    private void deleteCourse() {
        ...
        if (result) {
            ...
            if (deleteCourse == null) {
                outputView.printMessage("🚮 확인: " + id + "번 과정이 삭제 되었습니다.");
            } else {
