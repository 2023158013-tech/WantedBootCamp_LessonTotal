...
/*
    ...
    * 데이터의 일관성이 지켜지지 않았다고 표현한다.
    
    트랜잭션은 2개의 update구문을 하나의 논리적인 작업 단위로 묶어 두 작업이 모두 성공하거나 또는 하나라도 실패하게 되면 원상복구하도록 보장하는 기능이다.
    이 트랜잭션의 기능을 통해 데이터베이스는 어떤 상황이라도 데이터의 무결성과 일관성을 지킬 수 있다.
*/
...
start transaction; -- 아래는 모두 트랜잭션 반영됨
...
-- 중간 확인
-- 아직 우리 눈에만 확인이 되며 다른 사람에게는 반영이 안된 상태
select*from accounts;
