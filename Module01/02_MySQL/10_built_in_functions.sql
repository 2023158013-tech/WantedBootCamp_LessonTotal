-- 문자열, 숫자, 날짜, 조건 함수 등 가장 많이 쓰이는 내장 함수를 학습한다.
...
-- replace: 문자열의 일부를 다른 문자열로 바꾼다.(재배치)
select
	email,
...
-- (포맷팅 형식: %Y(4자리 년도), %m 월, %d 일, %H 24시, %i 분, %s 초)
-- 저런 포맷 형식은 실제 정규 표현식에 기반한다. 정규 표현식은 절대 외우지 말고 우리가 날짜를 변환해야할 때 정규표현식을 쓰면 어떨까?라는 생각만 가지면 된다.
select
	created_at,
...
-- user_profiles 테이블에서 bio(자기소개)가 null인 경우, null 대신 '자기소개서 미작성'이라고 표시해보자.
...
-- grades 테이블의 점수에 따라서 95점 이상 A+, 90점 A, 85점 이상 B+, 80점 이상 B, 70 C, 그 밑 D
-- 예상 결과: 조평훈 97 A+
-- 힌트: users 테이블과 grades 테이블 join, 별칭으로 각 컬럼 및 테이블 구분
-- case when then 구문으로 점수별 분리
select
	name,
    case
