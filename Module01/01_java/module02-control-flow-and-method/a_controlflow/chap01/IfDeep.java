...
public class IfDeep {
    ...
    public static void main(String[] args) {
        ...
        * AND 연산(&&):
        * -두 피연산자가 모두 참(true)일 때만 전체 표현식이 참이다!!<중요한 특징>
        * -첫 번째 피연산자가 false일 경우, 두 번째 피연산자를 평가할 필요가 없기 때문에 평가를 중단한다.
        *
        * OR 연산(||):
        * -두 피연산자 중 하나라도 참(true)일 경우  전체 표현식이 참이다!!
        * -첫 번째 피연산자가 true일 경우, 두 번째 피연산자를 평가할 필요가 없다.
        *
        * 메모리 관점
        * -조건식의 변수들은 스택에 저장이 되며, 너무 많은 중첩 if문은 소택 사용량을 늘리고 가독성 또한 떨어뜨린다.
        * */
        ...
        long startTime = System.nanoTime(); //시작 시간 체크(코드 실행 순서 알 수 있는 방법)

        if(age <= 19){ //드문 조건(우린 모두 대학생이라서 드뭄)
            discount = "학생 할인 가능";
        } else {
            discount = "할인 불가";
        }
        ...
        long startTime2 = System.nanoTime(); //시작 시간 체크(코드 실행 순서 알 수 있는 방법)
